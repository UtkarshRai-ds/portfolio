            ...
            if submitted:
                if name and email and message:
                    st.success(
                        ...
                        "I'll get back to you at the email provided.",
                        icon="✅",
                    )
                    st.info(
                        "💡 To activate the contact form, connect it to your email backend "
                        "(e.g. EmailJS, Formspree, or SMTP via `smtplib`).",
                        icon="ℹ️",
                    )
                else:
                ...
                <div style="display:flex;align-items:center;gap:12px;">
                    ...
                    <div>
                        <div style="font-size:11px;color:#8b949e;text-transform:uppercase;
                                    letter-spacing:1px;font-family:'Space Mono',monospace;">{label}</div>
                        <div style="font-size:14px;margin-top:2px;">{link_html}</div>
                    </div>
                </div>
            </div>
            """, unsafe_allow_html=True)
